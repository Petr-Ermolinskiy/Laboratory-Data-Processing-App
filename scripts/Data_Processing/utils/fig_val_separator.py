...
def apply_global_comma_patch() -> None:
    ...
    # охраняем оригинальный метод создания Axis
    _ORIGINAL_METHODS["Axis.__init__"] = mpl.axis.Axis.__init__

    # Создаем новый класс Axis, использующий наш форматтер
    class CommaAxis(_ORIGINAL_CLASSES["Axis"]):
        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
            if not isinstance(self.get_major_formatter(), UniversalCommaFormatter):
                self.set_major_formatter(CommaDecimalScalarFormatter())

    # Заменяем класс Axis
    mpl.axis.Axis = CommaAxis
